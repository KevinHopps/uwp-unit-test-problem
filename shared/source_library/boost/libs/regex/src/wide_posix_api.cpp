...
#pragma warning(disable:4267)
   ...
   if(result)
   {
      ...
      int i;
      for(i = 0; (i < int(n)) && (i < int(expression->re_nsub + 1)); ++i)
      {
         array[i].rm_so = int((m[i].matched == false) ? -1 : (m[i].first - buf));
         array[i].rm_eo = int((m[i].matched == false) ? -1 : (m[i].second - buf));
      }
      // and set anything else to -1:
      for(i = int(expression->re_nsub + 1); i < int(n); ++i)
      {
         array[i].rm_so = -1;
         array[i].rm_eo = -1;
      }
