...
namespace posix_time {
  ...
  operator>>(std::basic_istream<CharT, Traits>& is, ptime& pt)
  {
    ...
    if (strm_sentry) {
      try {
        typedef typename date_time::time_input_facet<ptime, CharT> time_input_facet_t;
        std::istreambuf_iterator<CharT,Traits> sit(is), str_end;
        if(std::has_facet<time_input_facet_t>(is.getloc())) {
          std::use_facet<time_input_facet_t>(is.getloc()).get(sit, str_end, is, pt);
        }
        else {
          time_input_facet_t* f = new time_input_facet_t();
  ...
  operator>>(std::basic_istream<CharT, Traits>& is, time_period& tp)
  {
    ...
    if (strm_sentry) {
      try {
        ...
        std::istreambuf_iterator<CharT,Traits> sit(is), str_end;
        if(std::has_facet<time_input_facet_t>(is.getloc())) {
          std::use_facet<time_input_facet_t>(is.getloc()).get(sit, str_end, is, tp);
        }
        else {
          time_input_facet_t* f = new time_input_facet_t();
          std::locale l = std::locale(is.getloc(), f);
  ...
  operator>>(std::basic_istream<CharT, Traits>& is, time_duration& td)
  {
    ...
    if (strm_sentry) {
      try {
        ...
        std::istreambuf_iterator<CharT,Traits> sit(is), str_end;
        if(std::has_facet<time_input_facet_t>(is.getloc())) {
          std::use_facet<time_input_facet_t>(is.getloc()).get(sit, str_end, is, td);
        }
        else {
          time_input_facet_t* f = new time_input_facet_t();
          std::locale l = std::locale(is.getloc(), f);
