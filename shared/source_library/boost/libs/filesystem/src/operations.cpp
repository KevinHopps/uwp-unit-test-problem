...
# if BOOST_PLAT_WINDOWS_DESKTOP
  ...
  static inline PtrRtlEqualUnicodeString rtl_equal_unicode_string_api() {
	  static PtrRtlEqualUnicodeString fn_ptr = PtrRtlEqualUnicodeString(
		  ::GetProcAddress(
...
#  define LOCALE_INVARIANT (MAKELCID(MAKELANGID(LANG_INVARIANT, SUBLANG_NEUTRAL), SORT_DEFAULT))
...
#else
	int res = ::LCMapStringEx(LOCALE_NAME_INVARIANT, LCMAP_UPPERCASE, &input, 1, &result, 1, NULL, NULL, 0);
#endif
	if (res != 0)
      return result;
  ...
  static inline Ptr_equal_string_ordinal_ic equal_string_ordinal_ic() {
	  static Ptr_equal_string_ordinal_ic fn_ptr=
# if BOOST_PLAT_WINDOWS_DESKTOP
  ...
  path canonical(const path& p, const path& base, system::error_code* ec)
  {
    ...
    while (scan)
    {
      ...
      for (path::iterator itr = source.begin(); itr != source.end(); ++itr)
      {
        ...
        // On Windows UWP, checking status of directories leading up to
        // the sandbox will fail, even though the thing referred to is
        // in the sandbox. Since we've already verified the existence
        // of the item, we ignore failures along the way.
        //
        bool check_symlink = true;
#ifdef BOOST_WINDOWS_API
        if (true)
            {
            check_symlink = false;
            file_status fs = status(result, &local_ec);
            file_type ft = fs.type();
            if (ft != status_error && ft != type_unknown)
                check_symlink = true;
            }
#endif
        bool is_sym = false;
        if (check_symlink) // may fail on UWP, even though we know the full path exists
            {
            is_sym = is_symlink(detail::symlink_status(result, ec));
            if (ec && *ec)
                return path();
            }
